...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="css/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
...
//								}
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					var fun = options.getOrderID();
					//
					if(fun){
						var order_id =  options.getOrderID();
						ZYFILE.url =  ZYFILE.url + "&order_id=" + order_id;
						// 判断当前是否有文件需要上传
						if(ZYFILE.funReturnNeedFiles().length > 0){
							$("#fileSubmit").click();
						}else{
							alert("没有要上传的文件，请先选中文件再点击上传");
						}
					}else{
						return;
					}
