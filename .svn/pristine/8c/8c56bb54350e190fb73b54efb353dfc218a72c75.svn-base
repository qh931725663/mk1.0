...
function member_add_check(){
    ...
    if($("#t2").is(":checked")){
        if($("#passport").val()==""){
            $("#passport_mes").html("请输入证件号！");
            member_add_flag=false;
        }else{
            $("#passport_mes").html("");
        }
        if($("#user_age").val()==""){
            $("#user_age_mes").html("请输入年龄！");
            member_add_flag=false;
        }else{
            $("#user_age_mes").html("");
        }
    ...
    if($("#user_sex option:selected").val()=='0'){
        $("#user_sex_mes").html("请输入性别！");
        member_add_flag=false;
    }else{
    	 $("#user_sex_mes").html("");
    }
...
function new_add_edit_check(){
    news_flag=true;
    var title=/^.{10,20}$/;
    var short_introduction=/^.{10,20}$/;
    if(!title.test($("#title").val())){
        $("#title_mes").html("必填项，请输入10到20字符！");
        news_flag=false;
    }else{
        $("#title_mes").html("");
    }
    if($("#pic").val()==""){
        $("#pic_mes").html("(请选择图片文件！)");
        news_flag=false;
    }else{
        $("#pic_mes").html("");
    }
    if($('#news-1').is(":checked")){
        if(!short_introduction.test($("#short_introduction").val())){
            $("#short_introduction_mes").html("必填项，请输入10到20字符！");
            news_flag=false;
        }else {
            $("#short_introduction_mes").html("");
        }
        if(editor1.html()==''){//获取编辑器内容 editor1.html()
            $("#the_text_mes").html("请输入正文内容！");
            news_flag=false;
        }else{
            $("#the_text_mes").html("");
        }
    }
    if($('#news-2').is(":checked")){
        if($("#url_index").val()==''){
            $("#url_index_mes").html("请输入链接地址！");
            news_flag=false;
        }else {
            $("#url_index_mes").html("");
        }
