...
public class BloodPressureDao extends EquipmentDao {
	...
	public List<Map<String, Object>> getDataByTime(long user_id, String datemin, String datemax) {
		...
		StringBuffer sql = new StringBuffer("SELECT A.order_id,A.create_date,B.HighPressure,B.LowPressure "
				+ " FROM c_equipment_use A,c_blood_pressure B "
	...
	public Page pageQueryByTime(long user_id, String datemin, String datemax, int pageno, int pagesize) {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT A.order_id,DATE_FORMAT(A.create_date,'%Y-%m-%d %H:%i') datetime,B.HighPressure,B.LowPressure,B.PulseRate "
						+ " FROM c_equipment_use A,c_blood_pressure B "
						+ " WHERE A.order_id=B.equipment_use_order_id AND A.user_id = ? ");
		list.add(user_id);
		if (datemin != null && !datemin.equals("") && datemax != null && !datemax.equals("")) {
	...
	public Map<String, Object> getMostRecentData(long user_id) {
		String sql = "SELECT a.HighPressure,a.LowPressure,a.PulseRate,DATE_FORMAT(a.create_date,'%Y-%m-%d %H:%i')datetime FROM c_blood_pressure a LEFT JOIN c_equipment_use b ON a.equipment_use_order_id = b.order_id WHERE b.user_id=? order by b.create_date desc limit 1";
	...
	//查询用户的管理计划
	public List<Map<String, Object>> getPlan(String patient_id) {
		String sql = "SELECT p.user_id user_id, p.order_id  plan_order_id,d.order_id dict_order_id "
				+ "FROM p_plan p LEFT JOIN p_plan_dict d ON  p.type =d.type AND p.plan_level = d.plan_level AND d.plan_name='血压监测' "
				+ "WHERE p.user_id=?";
		return jdbcTemplate.queryForList(sql,patient_id);
	}

	public void deleteBloodPressure(String order_id) {
		String sql = "DELETE FROM c_blood_pressure WHERE order_id =?;";
