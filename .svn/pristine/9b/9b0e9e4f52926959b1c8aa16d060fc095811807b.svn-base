...
public class SurveyService {
    ...
    public Map<String, Object> queryUserSurvey(String user_id){
       String sql = "select a.*, date_format(a.create_date,'%Y-%m-%d') date from s_survey a where user_id = ? order by a.create_date desc limit 1";
       List<Map<String, Object>> list = dao.select(sql, user_id);
    ...
    public Map<String, Object> queryUserInfo(String user_id){
        String sql = "select * from n_user where user_id = " + user_id;
        List<Map<String, Object>> list = dao.select(sql);
        return list.get(0);
    }
    
	public InfoJson getDetail(String user_id){
		InfoJson info = new InfoJson();
		Map<String,Map<String,Object>> survey = new HashMap<>();
		Map<String,Object> patientInfo = dao.getPatientInfo(user_id);
		if(patientInfo.get("user_card")!=null){
			...
			patientInfo.putIfAbsent("user_birthday", card.getBirthday());
			patientInfo.putIfAbsent("user_sex", card.getSex()%2==0?200002:200001);
		}
		survey.put("base", patientInfo);
		Object order_id = patientInfo.get("order_id");
		if(order_id!=null){
			...
			List<Map<String,Object>> surveyInfo= dao.getSurveyInfo(order_id.toString());
			List<Object> chronicList = surveyInfo.stream().filter(m->Integer.parseInt(m.get("parent_id").toString())==100).collect(Collectors.toList());
			List<Object> serviceList = surveyInfo.stream().filter(m->Integer.parseInt(m.get("parent_id").toString())==1500).collect(Collectors.toList());
			List<Object> healthList = surveyInfo.stream()
					.filter(m->Integer.parseInt(m.get("parent_id").toString())>=200)
					.filter(m->Integer.parseInt(m.get("parent_id").toString())<=1400)
					.collect(Collectors.toList());
			healthMap.put("list", healthList);
			chronicMap.put("list", chronicList);
			serviceMap.put("list", serviceList);
			survey.put("heal", healthMap);
			survey.put("service", serviceMap);
			survey.put("chronic", chronicMap);
			info.setData(survey);
		}else{
			info.setData(survey);
		}
