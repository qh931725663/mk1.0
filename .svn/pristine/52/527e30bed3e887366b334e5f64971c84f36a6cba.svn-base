...
public class ChronicPlanServiceImpl implements ChronicPlanService{
	...
	public void addPlanItems(List<Map> parseArray) {	
		...
		String check_time = String.valueOf(parseArray.get(0).get("check_time"));
		...
		Long item_order_id= chronicPlanDao.addItem(plan_order_id,plan_dic_order_id,user_id,create_date,create_by,check_time);	
		//采用批量提交测量表单的数据
	...
	//血压曲线图查询
	@Override
	public InfoJson queryRecent(Long user_id, Integer days) {
		InfoJson infoJson = new InfoJson();
		if (chronicPlanDao.queryRecent(user_id, 90).size() != 0) {
			infoJson.setCount(infoJson.getCount() + 1);
		}
		List<Map<String, Object>> list = chronicPlanDao.queryRecent(user_id, days);
		list.forEach(s -> {
			// 判断血压是否正常
	...
	//血压分页查询
	@Override
	public InfoJson pageQuery(Long user_id, String year_month, Integer pageno, Integer pagesize) {
		InfoJson infoJson = new InfoJson();
		...
		if (!StringUtil.isEmpty(year_month)) {
			dataList = chronicPlanDao.queryByMonth(user_id, year_month, pageno, pagesize);
			infoJson.setCount(chronicPlanDao.pageNumberByMonth(user_id, year_month, pagesize));
		} else {
			dataList = chronicPlanDao.pageQuery(user_id, pageno, pagesize);
			infoJson.setCount(chronicPlanDao.pageNumber(user_id, pagesize));
		}
		if (dataList.size() > 0) {
			Iterator<Map<String, Object>> iterator = dataList.iterator();
			Map<String, Object> map = iterator.next();
