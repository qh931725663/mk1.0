...
public class BloodOxygenDao extends EquipmentDao {
	...
	public List<Map<String, Object>> getDataByTime(long user_id, String datemin, String datemax) {
		...
		StringBuffer sql = new StringBuffer("SELECT A.order_id,A.create_date,B.Oxygen,B.PulseRate "
				+ " FROM c_equipment_use A,c_pulse_bloodoxygen B "
	...
	public Page pageQueryByTime(long user_id, String datemin, String datemax, int pageno, int pagesize) {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT A.order_id,DATE_FORMAT(A.create_date,'%Y-%m-%d %H:%i') datetime,B.Oxygen,B.PulseRate "
						+ " FROM c_equipment_use A,c_pulse_bloodoxygen B "
						+ " WHERE A.order_id=B.equipment_use_order_id AND A.user_id = ? ");
		list.add(user_id);
		if (datemin != null && !datemin.equals("") && datemax != null && !datemax.equals("")) {
	...
	public Map<String, Object> getMostRecentData(long user_id) {
		String sql = "SELECT a.Oxygen,a.PulseRate,DATE_FORMAT(a.create_date,'%Y-%m-%d %H:%i')datetime FROM c_pulse_bloodoxygen a LEFT JOIN c_equipment_use b ON a.equipment_use_order_id = b.order_id WHERE b.user_id=? order by b.create_date desc limit 1";
