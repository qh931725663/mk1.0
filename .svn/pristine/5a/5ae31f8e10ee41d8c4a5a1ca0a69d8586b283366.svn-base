...
public class DoctorDao extends BaseTemplateDao {
	...
	public int countByAccount(String account) {
		...
		if (count == 0) {
			count = this.getValue("select count(*) from n_user where user_phone = ? ", args, Integer.class);
		}

		if (count == 0) {
			count = this.getValue("select count(*) from n_user where user_card = ? ", args, Integer.class);
		}

		if (count == 0) {
			count = this.getValue("select count(*) from n_user where passport = ? ", args, Integer.class);
		}
		return count;
	}
	...
	public int countByAccount(String account, String user_pwd) {
		int count = 0;
		Object[] args = new Object[] { account, user_pwd, Constant.USER_TYPE_DOCTOR };

		if (count == 0) {
			count = this.getValue(
					"select count(*) from n_user where user_phone = ? and user_pwd = ? and user_type = ? ", args,
					Integer.class);
		}

		if (count == 0) {
			count = this.getValue("select count(*) from n_user where user_card = ? and user_pwd = ? and user_type = ? ",
					args, Integer.class);
		}

		if (count == 0) {
			count = this.getValue("select count(*) from n_user where passport = ? and user_pwd = ? and user_type = ? ",
					args, Integer.class);
		}
		return count;
	}
	...
	public String getUserId(String account, String user_type) {
		int count = 0;
		Object[] arg = new Object[] { account };
		Object[] args = new Object[] { account, user_type };

		if (count == 0) {
			count = this.getValue("select count(*) from n_user where user_phone = ? ", arg, Integer.class);
			if (count > 0) {
				return this.getValue("select user_id from n_user where user_phone = ? and user_type = ? ", args,
						String.class);
			}
		}
		if (count == 0) {
			count = this.getValue("select count(*) from n_user where user_card = ? ", arg, Integer.class);
			if (count > 0) {
				return this.getValue("select user_id from n_user where user_card = ? and user_type = ? ", args,
						String.class);
			}
		}
		if (count == 0) {
			count = this.getValue("select count(*) from n_user where passport = ? ", arg, Integer.class);
			if (count > 0) {
				return this.getValue("select user_id from n_user where passport = ? and user_type = ? ", args,
						String.class);
			}
		}
		return null;
	...
	public Map<String, Object> getDoctorInfo(String doctor_id) {
		...
		String sql = "select a.user_id,a.user_type,a.user_name,a.user_phone,a.user_card,a.user_sex,a.user_age,c.hosp_order_id,d.hosp_name,"
	...
	public Map<String, Object> getPatientInfo(String patient_id) {
		...
		if (patientInfo.get("mainland").toString().equals("1")) {
			Object user_card = patientInfo.get("user_card");
			if (user_card != null) {
		...
		if (patientInfo.get("user_blood") == null) {
			patientInfo.put("user_blood", "");
		}
