...
public class BloodSugarDao extends EquipmentDao {
	...
	public Map<String, Object> getMostRecentData(long user_id) {
		String sql = "SELECT a.BloodSugar,a.measurement_period,DATE_FORMAT(a.create_date,'%Y-%m-%d %H:%i')datetime FROM c_blood_sugar a LEFT JOIN c_equipment_use b ON a.equipment_use_order_id = b.order_id WHERE b.user_id=? order by b.create_date desc limit 1";
	...
	//查询用户的管理计划 :这里需要根据用户的测量时间 对应管理计划dic表里面的两种检测时段进行区分
	public List<Map<String, Object>> getPlan(String patient_id,String period) {
		if (Integer.parseInt(period) == Constant.EMPTY_STOMACH) {
			String sql = "SELECT p.user_id user_id, p.order_id  plan_order_id,d.order_id dict_order_id "
					+ "FROM p_plan p LEFT JOIN p_plan_dict d ON  p.type =d.type AND p.plan_level = d.plan_level AND d.plan_name='空腹血糖检测' "
					+ "WHERE p.user_id=?";	
			return jdbcTemplate.queryForList(sql,patient_id);
		}else if (Integer.parseInt(period) == Constant.AFTER_BREAKFAST || Integer.parseInt(period) == Constant.AFTER_DINNER
				|| Integer.parseInt(period) == Constant.AFTER_LUNCH) {
			String sql = "SELECT p.user_id user_id, p.order_id  plan_order_id,d.order_id dict_order_id "
					+ "FROM p_plan p LEFT JOIN p_plan_dict d ON  p.type =d.type AND p.plan_level = d.plan_level AND  d.plan_name='餐后2h血糖检测'  "
					+ "WHERE p.user_id=?";	
			return jdbcTemplate.queryForList(sql,patient_id);
		}
		return null;		
