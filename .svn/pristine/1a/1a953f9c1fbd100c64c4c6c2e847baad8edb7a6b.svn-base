...
public class BloodSugarService {
	...
	@Transactional
	@Log(name = "血糖记录添加")
	public InfoJson add(EquipmentUse equipmentUse, BloodSugar bloodSugar,String create_by) {
		...
		//管理向content里面插入数据计划
		insetIterm(equipmentUse.getUser_id(),equipmentUse.getCreate_date(),bloodSugar.getMeasurement_period(),create_by);
	 ...
	 * @param create_by
	 */
	private void insetIterm(String patient_id, String create_date,String period, String create_by) {
		//获取用户的管理计划
	...
	private void pushUrgentMessage(String patient_id, String doctor_id, BloodSugar bloodSugar) {
		...
		if (level > 0 && String.valueOf(1).equals(patientMap.get("is_send"))) {
			...
			content = content + Constant.MEASUREMENT_PERIOD_MAP.get(bloodSugar.getMeasurement_period()) + "\40\40"
					+ "血糖：" + bloodSugar.getBloodSugar() + "mmol/L";
	 ...
	 * 
	 * @Title: queryRecent 
	 * @Description: app端查询曲线数据 (android IOS) 
	 * @param  user_id 
	 * @param period 
	 * @return InfoJson 返回类型
	 * @throws
	 ...
	 * 
	 * @Title: queryRecent
	 * @Description: app端查询曲线数据（IOS） 
	 * @param user_id 
	 * @param period 
	 * @return InfoJson 返回类型 @throws
	 */
	 ...
	 * 
	 * @Title: classify 
	 * @Description: 通过传入的血糖值以及测量时段判断血糖的严重程度 
	 * @param blood_sugar 
	 * @param period 
	 * @return 
	 * @throws
