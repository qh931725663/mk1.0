...
public class AIODao extends BaseTemplateDao {
	...
	public List<Map<String, Object>> getTimeList(String user_id, String datemin, String datemax) {
		...
		if (datemin != null && !datemin.equals("") && datemax != null && !datemax.equals("")) {
			sql += " DATE_FORMAT(create_date,'%Y-%m-%d')>=? AND DATE_FORMAT(create_date,'%Y-%m-%d') <= ? ORDER BY create_date DESC";
			list = jdbcTemplate.queryForList(sql, user_id, datemin, datemax);
		} else if (datemin != null && !datemin.equals("")) {
			sql += " DATE_FORMAT(create_date,'%Y-%m-%d') >= ? ORDER BY create_date DESC";
	...
	public List<Map<String, Object>> getDetailByTime(String user_id, String datemin, String datemax) {
		List<Map<String, Object>> list;
		StringBuffer sql =new StringBuffer("select order_id,user_name,equipment_property_order_id,FORMAT(user_height,1)user_height,FORMAT(user_weight,1)user_weight,FORMAT(BMI,2)BMI,FORMAT(HighPressure,0)HighPressure,"
				+ "FORMAT(LowPressure,0)LowPressure,FORMAT(PulseRate,0)PulseRate,FORMAT(BloodSugar,1)BloodSugar,ecg1,ecg2,ecg3,ecg4,ecg5,ecg6,ecg7,ecg8,ecg9,ecg10,ecg11,ecg12,ecg_result,"
				...
				+ "DATE_FORMAT(create_date,'%Y-%m-%d') create_date,s6500_pic_index from v_self_service_check where user_id=? AND") ;
		if (datemin != null && !datemin.equals("") && datemax != null && !datemax.equals("")) {
			sql.append(" DATE_FORMAT(create_date,'%Y-%m-%d')>=? AND DATE_FORMAT(create_date,'%Y-%m-%d') <= ? ORDER BY create_date DESC");
			list = jdbcTemplate.queryForList(sql.toString(), user_id, datemin, datemax);
		} else if (datemin != null && !datemin.equals("")) {
			sql.append(" DATE_FORMAT(create_date,'%Y-%m-%d') >= ? ORDER BY create_date DESC");
			list = jdbcTemplate.queryForList(sql.toString(), user_id, datemin);
		} else if (datemax != null && !datemax.equals("")) {
			sql.append(" DATE_FORMAT(create_date,'%Y-%m-%d') <= ? ORDER BY create_date DESC");
			list = jdbcTemplate.queryForList(sql.toString(), user_id, datemax);
		} else {
			sql.append(" DATE_SUB(NOW(),INTERVAL 6 MONTH) <= create_date ORDER BY create_date DESC ") ;
	...
	public Map<String, Object> getAppLatestDetail(String user_id) {
		List<Map<String, Object>> list;
		String sql = "select equipment_property_order_id,FORMAT(user_height,1)user_height,FORMAT(user_weight,1)user_weight,FORMAT(BMI,2)BMI,FORMAT(HighPressure,0)HighPressure,"
				+ "FORMAT(LowPressure,0)LowPressure,FORMAT(PulseRate,0)PulseRate,FORMAT(BloodSugar,1)BloodSugar,ecg1,ecg2,ecg3,ecg4,ecg5,ecg6,ecg7,ecg8,ecg9,ecg10,ecg11,ecg12,"
				+ "findName(measurement_period) measurement_period,FORMAT(temperature,1)temperature,FORMAT(HeartRate,0)HeartRate,"
				+ "findName(result)result,FORMAT(fev1,2)fev1,FORMAT(fvc,2)fvc,FORMAT(pef,2)pef,"
				+ "FORMAT(Oxygen,0)Oxygen,findName(URO)URO,findName(BLD)BLD,findName(BIL)BIL,findName(KET)KET,findName(GLU)GLU,findName(PRO)PRO,"
				+ "FORMAT(PH,1)PH,findName(NIT)NIT,findName(LEU)LEU,FORMAT(SG,3)SG,findName(VC)VC,"
	...
	public Number insertAndGetKey(final Object object,String table) {
		...
		final Map<String, Object> data = map;
		return insertAndGetKey(data, table);
	}
	
	public Number insertAndGetKey(final Map<String, Object> data,String table) {
		KeyHolder keyHolder = new GeneratedKeyHolder();
		Map<String, Object[]> property = this.getParamsAndValues(data);
		Object[] params = property.get("params");
		Object[] values = property.get("values");
