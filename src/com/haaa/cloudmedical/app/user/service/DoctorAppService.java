...
public class DoctorAppService {
	...
	public ResponseDTO login(String user_phone, String user_pwd) {
		...
		int count = dao.countByAccount(user_phone,user_pwd);
		if (count == 0) {
	...
	// 获取患者列表
	public ResponseDTO getPatientList(String doctor_id, String ahdi_level, String chronic,String level) {
		...
		sql.append("select a.user_id,a.user_name,a.user_phone,a.user_card,a.passport,"
				+ "a.user_sex,a.user_age,a.user_height,a.user_weight,a.user_blood,concat('" + url
				+ "',a.user_head_pic_upload_index) user_head_pic_upload_index,a.mainland,a.user_birthday,"
		...
		if(level!=null){
			if(chronic.equals("1")){
				sql.append(" and b.level1 = ? ");
				values.add(level);
			}
			if(chronic.equals("2")){
				sql.append(" and b.level2 = ? ");
				values.add(level);
			}
		}
		...
		patientList.forEach(m -> {
			String mainland = m.get("mainland").toString();
			if (mainland.equals("1")) {
				Object user_card = m.get("user_card");
				if (user_card != null) {
					IdCard card = IdCard.of(user_card.toString());
					if (card.flag) {
						m.put("user_age", card.getAge() + "");
						m.put("user_birthday", card.getBirthday());
						m.put("user_sex", card.getSex() == 1 ? "200001" : "200002");
					}
				}
			}
			if (mainland.equals("0")) {
				Object user_birthday = m.get("user_birthday");
	...
	@Log(name = "添加非大陆患者")
	@Transactional
	public ResponseDTO addNewPatient(String doctor_id, String user_name, String user_phone, String passport,String user_sex, String user_birthday,String country,String country_name) {
		ResponseDTO dto = new ResponseDTO();
		int size1 = dao.countByAccount(user_phone);
