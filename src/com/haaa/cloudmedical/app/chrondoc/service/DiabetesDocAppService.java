...
public class DiabetesDocAppService {
	...
	public InfoJson addAnswers(Map<String, List<Object>> answers) {
		...
		System.out.println(diabetesDoc);
		int result = diabetesDocDao.updateDoc(diabetesDoc);
		if (result > 0) {
			/*String patient_id = diabetesDocDao.getValue("select user_id from t_chronic_dm where order_id = ?", new Object[]{order_id}, String.class);
			diabetesDocDao.callProcedure(patient_id, order_id);
			diabetesDocDao.track(patient_id);*/			
	...
	@Transactional
	public InfoJson init(Map<String, Object> diabetesDoc,String order_id){
		String patient_id = diabetesDocDao.getValue("select user_id from t_chronic_dm where order_id = ?", new Object[]{order_id}, String.class);
		diabetesDocDao.callProcedure(patient_id, order_id);
		diabetesDocDao.track(patient_id);			
		return InfoJson.setSucc(diabetesDoc);
	...
	public List<Map<String,Object>> getDocList(String patient_id){
		return diabetesDocDao.getDocList(patient_id);
