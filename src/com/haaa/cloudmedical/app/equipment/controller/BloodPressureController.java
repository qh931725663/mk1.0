...
public class BloodPressureController {
	...
	public InfoJson add(EquipmentUse equipmentUse, BloodPressure bloodPressure, HttpServletRequest request) {
		...
		try {
			...
			if (bloodPressure.getHighPressure() != null && bloodPressure.getHighPressure() != null
				...
				infoJson = service.add(equipmentUse, bloodPressure, create_by);
	 ...
	 */
	@RequestMapping(value = { "/deleteBloodPressure.action" }, method = { RequestMethod.GET })
	private InfoJson deleteBloodPressure(String order_id) {
		InfoJson infoJson = new InfoJson();
		if (order_id==null) {
		...
		try {
			service.deleteBloodPressure(order_id);
			infoJson.setStatus(1);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
