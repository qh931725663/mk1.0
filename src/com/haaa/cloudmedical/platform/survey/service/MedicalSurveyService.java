...
public class MedicalSurveyService {
	 ...
	 */
	@Log(name = "健康调查添加")
	public InfoJson saveHealthSurvey(Map<String, Object> data) throws Exception {
		InfoJson infoJson = new InfoJson();
		Date date = new Date();
		long order_id = saveHealthSurveyHistory(data, date);
		Set<String> questionSet = data.keySet();
		for (String question_id : questionSet) {
			Object answer = String.valueOf(data.get(question_id));
			Map<String, Object> questionMap = medicalSurveyDao.getQuestionById(question_id);
			String parent_id = String.valueOf(questionMap.remove("parent_id"));
			if (answer != null && !answer.equals("")) {
				if (questionSet.contains(parent_id)) {
					if (!question_id.equals("200") && parent_id.equals("200") && answer.equals("是")) {
						throw new RuntimeException();
					} else {
						saveAnswer(question_id, answer, date, order_id, questionMap);
					}
				} else {
					if (Integer.valueOf(parent_id) > 200 && Integer.valueOf(parent_id) < 300
							|| Integer.valueOf(parent_id) > 500 && Integer.valueOf(parent_id) < 600) {
						saveAnswer(question_id, answer, date, order_id, questionMap);
					} else {
						throw new RuntimeException();
					}
	...
	private void saveAnswer(String question_id, Object answer, Date date, Long order_id,
		...
		long question_order_id = medicalSurveyDao.insertAndGetKey(questionMap,"s_medical_history_question");
		if (answer instanceof List) {
			...
			for (Object object : answerList) {
				...
				medicalSurveyDao.insertAndGetKey(answerMap,"s_medical_history_answer");
			}
		} else {
			Map<String, Object> answerMap = new HashMap<String, Object>();
			answerMap.put("medical_history_order_id", order_id);
	...
	private long saveHealthSurveyHistory(Map<String, Object> data, Date date) {
		...
		return medicalSurveyDao.insertAndGetKey(survey_history,"s_medical_history");
	}
			...
			for (Map<String, Object> answerMap : answerList) {
				if (answerMap.get("question_index").equals(0)) {
					if (allAnswerMap.containsKey(String.valueOf(answerMap.get("question_k_order_id")))) {
						allAnswerMap.put("" + answerMap.get("question_k_order_id"),
