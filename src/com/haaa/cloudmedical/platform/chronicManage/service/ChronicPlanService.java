...
public class ChronicPlanService {
    ...
    public Page queryPage(User user, String doctor_name, ChronicManage model, int pageNo, String datemin, String datemax) {
        ...
        String sql_info = "select a.user_id, a.user_name,a.user_card,a.user_phone, findname(a.user_sex) user_sex,a.user_age,findname(a.user_marriage) user_marriage,getahdi(a.ahdi_value) ahdi_value,"
        ...
        //
        sql_info = "select * from (" + sql.toString() + ") m";
        ...
        if (StringUtils.isNotBlank(temp)) {
            if ("101".equals(temp)) {
                values.add(temp);
                sql_info += " where m.is_end = ? ";
            } else {
                values.add("%" + temp + "%");
                sql_info += " where m.is_end like ? or m.is_end is null ";
            }
        }
        //查询
        Page page = dao.pageQuery(sql_info, values.toArray(), pageNo);
