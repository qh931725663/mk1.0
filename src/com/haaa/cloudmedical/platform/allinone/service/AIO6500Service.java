	...
	public String saveData(AIO6500 aio) throws ParseException {
		...
		if (aio.getBloodsuger() != null || aio.getDia() != null || aio.getHeartrate() != null || aio.getMean() != null
			...
			EquipmentUse equipmentUse = new EquipmentUse(null, Constant.SELFSERVICE_6500,
					String.valueOf(user.get("user_id")), null, "selfservice6500", aio.getId(), null, DateUtil.dateFormat(date, "yyyy-MM-dd HH:mm:ss"), null,
			...
			if ((aio.getSys() != null && !aio.getSys().equals("error"))
				...
				BloodPressure bloodPressure = new BloodPressure(null, String.valueOf(use_id),
						...
						DateUtil.dateFormat(date, "yyyy-MM-dd HH:mm:ss"), null);
				bloodPressureDao.add(bloodPressure);
				//当血压数据有值的时候,增加管理计划的iterm表记录   createBy设置的设备的服务号码
				insetBloodPressureIterm(equipmentUse.getUser_id(),equipmentUse.getCreate_date(),Constant.SELFSERVICE_6500);
			}
			...
			if (aio.getBloodsuger() != null && !aio.getBloodsuger().equals("error")) {
				BloodSugar bloodSugar = new BloodSugar(null, String.valueOf(use_id),
						String.valueOf(Constant.EMPTY_STOMACH), aio.getBloodsuger(), DateUtil.dateFormat(date, "yyyy-MM-dd HH:mm:ss"), null);
				bloodSugarDao.add(bloodSugar);
				//管理向content里面插入数据计划  createBy设置的设备的服务号码
				insetBloodSugarIterm(equipmentUse.getUser_id(),equipmentUse.getCreate_date(),bloodSugar.getMeasurement_period(),Constant.SELFSERVICE_6500);
			}
			if (aio.getHeartrate()!= null && !aio.getHeartrate().equals("error")) {
				Electrocardiograph electrocardiograph = new Electrocardiograph(null, String.valueOf(use_id), aio.getHeartrate(), null, null, null, null, null, null, DateUtil.dateFormat(date, "yyyy-MM-dd HH:mm:ss"), null);
	...
	public String personalInfo(HttpServletRequest request) throws FileUploadException, IOException {
		...
		while (items.hasNext()) {
			...
			if (!item.isFormField()) {
				...
				EquipmentUse equipmentUse = new EquipmentUse(null, Constant.SELFSERVICE_6500,
						String.valueOf(user.get("user_id")), null, "selfservice6500", null, null, DateUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"), null,
						Constant.BLUETOOTHUPLOAD);
