...
public class RoleManageService {
     ...
     * @param data
     * @return 
     * @date: 2017年7月6日 上午9:37:30
     */
    public Object getRoleBindUser(Map<String, String> data) {
        if (StringUtil.isBlank(data.get("order_id"))) {
            return new RetData<Object>(false, "角色ID不能为空");
        }
        String sql = "select a.*,b.role_name from n_manager a left join n_role b on a.role_code = b.order_id where role_code=" + data.get("order_id");
        StringBuffer buf = new StringBuffer(sql);
        List<Object> params = new ArrayList<Object>();
        //电话
        if (StringUtil.isNotBlank(data.get("user_phone"))) {
            buf.append(" and user_phone like ? ");
            params.add("%" + data.get("user_phone") + "%");
        }
        //姓名
        if (StringUtil.isNotBlank(data.get("user_name"))) {
            buf.append(" and user_name like ? ");
            params.add("%" + data.get("user_name") + "%");
        }
        buf.append(" order by create_date desc");
        //显示页数
        if (StringUtil.isBlank(data.get("pageno"))) {
            data.put("pageNo", "1");
        }
        Page page = roleDao.pageQuery(buf.toString(), params.toArray(), Integer.valueOf(data.get("pageNo")));
        return new RetData<Page>(true, page);
    }
