...
public class ChronicPlanDao extends BaseTemplateDao {
	...
	public List<Map<String, Object>>  getAllIterms(String plan_order_id,String dict_order_id) {
		String sql ="SELECT i.order_id item_order_id,date_format(i.check_time,'%Y-%m-%d') create_time "
				+ "FROM p_plan_item i "
	 ...
	 * @return
	 */
	public Long addItem(String plan_order_id,String plan_dic_order_id,String user_id,String create_date,String create_by,String check_time) {
		long key = 0;
		String sql ="INSERT INTO p_plan_item (plan_order_id,dict_order_id,user_id,create_date,create_by,check_time)  VALUES (?,?,?,?,?,?)";
		System.out.println(sql);
		jdbcTemplate.update(sql.toString(), plan_order_id,plan_dic_order_id,user_id,create_date,create_by,check_time);
	...
	public List<Map<String, Object>> pageQuery(long user_id,int period,int pageno, int pagesize) {
		...
		StringBuffer sql = new StringBuffer( "SELECT B.equipment_use_order_id,A.order_id,A.create_date datetime,B.BloodSugar BloodSugar,findName(B.measurement_period) period "
				+ " FROM c_equipment_use A,c_blood_sugar B "
				+ " WHERE A.order_id=B.equipment_use_order_id AND A.user_id = ? AND B.create_date>DATE_SUB(CURDATE(), INTERVAL 1 YEAR)");
