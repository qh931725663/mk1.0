...
public class SmokingDao extends BaseTemplateDao{
	...
	public 	Map<String,Object> selectUserInfo(Long user_id){
	   	String sql="select getahdi(ahdi_value) ahdi_value,findname(user_marriage) user_marriage,user_id ,user_name ,user_card ,user_phone ,findname(user_is_vip) user_is_vip ,findname(user_sex) user_sex ,user_age ,findname(user_blood) user_blood ,findname(user_medical_burden) user_medical_burden ,user_medicare_card ,findname(user_work) user_work ,user_height ,user_weight ,date_format(user_create, '%Y-%m-%d  %H:%i:%s') user_create,smoked_year ,cigarettes_per_day ,doctor_id ,doctor_name ,order_id ,findname(quit_smoking_plan_end) quit_smoking_plan_end ,date_format(q_plan_create, '%Y-%m-%d  %H:%i:%s') q_plan_create from q_smoking_platform_view where user_id=? and order_id=(select max(order_id) from q_smoking_platform_view where user_id=?);";
	   	
	   	Map<String,Object> map =  this.jdbcTemplate.queryForMap(sql,user_id,user_id);
		return map;
	}
}
